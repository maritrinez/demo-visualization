(function () {
  ...
  const containers = {
          visitors: {
            ...
            header: '#visitors .chart-header'
          },
          revenue: {
            chart: '#revenue .chart',
            header: '#revenue .chart-header'
          },
          demand: {
            chart: '#demand .chart',
            header: '#demand .chart-header'
          }
        },
  ...
  const timeDayBreaks = hours(timeParse('%H:%M:%S')('0:0:0'), timeParse('%H:%M:%S')('23:59:59'), 6),
        timeDayLabels = ['evening', 'night', 'morning', 'afternoon'];

  // d3 imports = undefined;

  // // Constants
  // const formatDay= timeFormat('%u');

  // const wendTexture = textures
  //   .lines()
  //   .thicker()
  //   .stroke('lightgrey');

  // // TODO::hacer una textura para cada color ¿?
  // const eventTexture = textures
  //   .lines()
  //   .size(10)
  //   .strokeWidth(6);
  ...
  class Renders {
    ...
    slider(sld) {
      const dx = (this._size.w - this._size.slider) / 2;
      select('.sliderContainer').remove();
      const _g = this._enterGroup(this._svgRoot(), 'sliderContainer');
      _g.attr('transform', `translate(${dx}, ${40})`);
      _g.call(sld);

      // Add slider action text
      let _a = select('.sliderContainer').selectAll('.action').data([{}]);
      _a = _a.enter()
        .append('text').merge(_a)
        .attr('class', 'action')
        .attr('x', 0)
        .attr('y', 0)
        .text('Set a static price ↔');
      _a.exit().remove();


      // Add a circle to the handle to follow the design
      let _c = select('.parameter-value').selectAll('circle.handle').data([{}]);
      _c = _c.enter()
        .append('circle').merge(_c)
        .attr('class', 'handle')
        .attr('cx', 0)
        .attr('cy', 0)
        .attr('r', 4);
      ...
      select('.sliderContainer').selectAll('.tick')
        .attr('class', (d, i) => `tick t-${i}`);
    }


    axis(axs) {
      Object.keys(axs).map(a => {
        let _g = this._svg().selectAll(`g.${a}.axis`).data([{}]);
        _g = _g.enter()
          .append('g').merge(_g)
          .attr('class', `chart axis ${a}`)
          .attr('transform', axs[a].translate);
        _g.call(axs[a].axe);
      });
    }
    ...
    lines(linesData) {
      const _g = this._enterGroup(this._svg(), 'revenue');
      let _l = _g.selectAll('path').data(linesData);
      _l = _l.enter()
        .append('path').merge(_l)
        .attr('class', d => `line ${d[0]}`)
        .attr('d', d => this._pathCreator()(d[1]))
        .attr('fill', 'none');
        
      _g.selectAll('path').exit().remove();
    }

    linesGeneric(linesData, classed) {
      const _g = this._enterGroup(this._svg(), classed);
      let _l = _g.selectAll('path').data(linesData);
      _l = _l.enter()
        .append('path').merge(_l)
        .attr('class', d => (d.classed) ? `line ${d.classed}` : 'line')
        .attr('d', d => this._pathCreator()(d.values))
        .attr('fill', 'none');
        
      _g.selectAll('path').exit().remove();
    }

    rectsRounded(data, classed) {
      const _g = this._enterGroup(this._svg(), classed);
      let _l = _g.selectAll('path').data(data);
      _l = _l.enter()
        .append('path').merge(_l)
        .attr('class', d => (d.classed) ? `rect ${d.classed}` : 'rect')
        .attr('d', d => _topRoundedRect(d.x, d.y, d.width, d.height, 3));
        
      _g.selectAll('path').exit().remove();

      function _topRoundedRect (x, y, width, height, radius) {
        return `M${x + radius},${y}
      h${width - radius * 2}
    ...
    areas(areasData) {    
      const _g = this._enterGroup(this._svg(), 'revenue_diff');
      let _l = _g.selectAll('path').data([areasData]);
      _l = _l.enter()
        .append('path').merge(_l)
        .attr('class', 'area diff')
        .attr('d', d => this._areaCreator()(d));
    ...
    text(data, classed) {    
      ...
      const bbox = this._svg().node().getBoundingClientRect(), 
            dy = bbox.top + window.scrollY - 20,
            dx = bbox.left + 15; // div width / 2

      
      let _l = select(this._container).selectAll(`div .${classed}`).data(data);
      _l = _l.enter()
        .append('div').merge(_l)
        .attr('class', classed)
        .html(d => d.txt)
        .style("left", d => d.x + dx + "px")
        .style("top", d => d.y + dy + "px");
        
      select(this._container).selectAll(`div .${classed}`).exit().remove();
    }

    addTexture (texture) {
      this._svg().call(texture);
    }

    addShadowFilter() {
      let _d = this._svg().selectAll('defs').data([{}]);
      _d = _d.enter().append('defs').merge(_d);
      _d.selectAll('defs').exit().remove();

      let _f = this._svg().select('defs').selectAll('filter').data([{}]);
      _f = _f.enter()
        .append('filter').merge(_f)
          .attr('id', 'shadow')
          .attr('x', '-50%')
          .attr('y', '-50%')
          .attr('width', '250%')
          .attr('height', '250%');
      _f.selectAll('filter').exit().remove();

      let _s = _f.selectAll('feDropShadow').data([{}]);
      _s = _s.enter()
        .append('feDropShadow')
          .attr('dx', 0)
          .attr('dy', 0)
          .attr('stdDeviation', 2)
          .attr('flood-color', '#A5A6AE');
      _s.selectAll('feDropShadow').exit().remove();
    }


    daytime (data) {
      const _g = this._enterGroup(this._svg(), 'daytime');
      // lines
      let _l = _g.selectAll('line').data(data.filter(d => d.key == 'line')[0].values);
      _l = _l.enter()
      ...
      _g.selectAll('line').exit().remove();

      // labels
      let _t = _g.selectAll('text.label').data(data.filter(d => d.key == 'label')[0].values);
      _t = _t.enter()
        .append('text').merge(_t)
        .attr('class', 'label')
        .attr('x', d => d.x)
        .attr('y', d => d.y)
        .text(d => d.txt)
      ...
      _g.selectAll('tetext.labelxt').exit().remove();

      // difs
      let _d = _g.selectAll('text.dif').data(data.filter(d => d.key == 'dif')[0].values);
      _d = _d.enter()
        .append('text').merge(_d)
        .attr('class', 'dif')
        .attr('x', d => d.x)
        .attr('y', d => d.y)
        .text(d => d.txt)
      ...
      _g.selectAll('text.dif').exit().remove();
    ...
    // - - - PRIVATE FUNCTIONS - - - //
    _svgRoot () {
      return select(this._container).selectAll('svg');
    }
  ...
  class Tooltip {
    ...
    init() {
      ...
      const tipGroup = this._enterGroup(select(this._container).select('svg').select('g'), 'ttip tip-elements');

      // Append an overlay on top for capture the events
      let overlay = this._rects(select(this._container).select('svg').select('g'), this._overlayData());
      overlay
        .attr('class', 'overlay')
    ...
    _overlayData() {
      return [
        {x: 0, y: 0, width: this._size.w, height: this._size.h}
      ]
    }


    _mouseover() {
      selectAll('.ttip').style('display', null);
    }


    _mouseout() {
      selectAll('.ttip').style('display', 'none');
    }
    ...
    _mousemove() {
      ...
      // get data
      const _bisect = bisector(d => d.time).left;
      var x0 = this._scales.x.invert(pointer(event,this)[0] - bbox.left),
          i = _bisect(this._data, x0, 1),
          d0 = this._data[i - 1].time,
          d1 = this._data[i].time,
          d = x0 - d0.time > d1.time - x0 ? d1 : d0,
          datum = this._data.filter(v => v.time == d);

      
      // Append v line
      const _lineData = [{x1:this._scales.x(d) ,y1:0 ,x2:this._scales.x(d), y2: this._size.h}];
      this._line(select(this._container).select('.tip-elements'), _lineData);


      // build data to print the elements
      if (this._chart == 'revenue') {
        const dyDown = -50,
              dyUp = 25;
        this._scaledData = datum.map(d => {
            return {
              x: this._scales.x(d.time),
              y: this._scales.y(d.revenue),
              r: 6,
              dx: bbox.left - margin.left + 15,
              dy: d.price == 'dynamic' ? bbox.top + window.scrollY + dyDown : bbox.top + window.scrollY + dyUp,
              classed: d.price == 'dynamic' ? `down ${d.price}` : `up ${d.price}`,
              txt: format('($.0f')(d.revenue)
            }
        }); 
      } else {
        this._scaledData = [];
        datum.map(d => {
          this._scaledData.push( 
            {
              x: this._scales.x(d.time),
              y: this._scales.yLeft(d.visitors),
              r: 6,
              dx: bbox.left - margin.left + 15,
              dy: bbox.top + window.scrollY - 50,
              classed: 'down visitors',
              txt: format('(.0f')(d.visitors)
            },
            {
              x: this._scales.x(d.time),
              y: this._scales.yRight(d.conversion),
              r: 6,
              dx: bbox.left - margin.left + 15,
              dy: bbox.top + window.scrollY + 25,
              classed: 'up conversion',
              txt: format('(.1%')(d.conversion), 
            }
          );
        }); 
      }
      ...
      // Append elements
      this._circles(select(this._container).select('.tip-elements'), this._scaledData);
      this._divs(select(this._container), this._scaledData); 
    }

    _divs(selection, datum) {

      let t = selection.selectAll('.tip-pop').data(datum);
      t = t.enter()
        .append('div').merge(t)
        .attr('class', d => (d.classed) ? `ttip tip-pop  ${d.classed}` : 'ttip tip-pop')
        .html(d => d.txt)
        .style("left", d => d.x + d.dx + "px")
        .style("top", d => d.y + d.dy + "px");
    ...
    _enterGroup(selection, classed) {
      let _g = selection.selectAll(`g.tip-elements`).data([{}]);
    ...
    _circles(selection, datum) {
      let _g = selection.selectAll('circle').data(datum);

      _g = _g.enter()
        .append('circle').merge(_g)
          .attr('class', d => (d.classed) ? d.classed : '')
          .attr('cx', d => d.x)
          .attr('cy', d => d.y)
          .attr('r', d => d.r);
    ...
    _line(selection, datum) {
      let _g = selection.selectAll('line').data(datum);

      _g = _g.enter()
        .append('line').merge(_g)
          .attr('class', d => (d.classed) ? d.classed : '')
          .attr('x1', d => d.x1)
          .attr('y1', d => d.y1)
          .attr('x2', d => d.x2)
    ...
    _rects(selection, datum) {
      let _g = selection.selectAll('rect').data(datum);

      _g = _g.enter()
        .append('rect').merge(_g)
          .attr('class', d => (d.classed) ? d.classed : '')
          .attr('x', d => d.x)
          .attr('y', d => d.y)
  ...
  class Header {
    ...
    // - - - PUBLIC FUNCTIONS - - - //
    init() {
      const main = this._divs(select(this._container), 'main');
      this._divs(main, 'title', this._title);
      if (this._subtitle) this._divs(main, 'subtitle', this._subtitle);
      
      const legend = this._divs(select(this._container), 'legend');
      const cells = this._divs(legend, 'cell', this._headerData);
      this._divs(cells, 'details', d => d.details);
      this._divs(cells.selectAll('.figures'), 'value',d => d.values);
    }


    // - - - PRIVATE FUNCTIONS - - - //
    // 
    _divs(selection, classed, datum) {
      if (!datum) datum = [{}];

      let t = selection.selectAll(`div.${classed}`).data(datum);
      t = t.enter()
        .append('div').merge(t)
        .attr('class', d => (d.classed) ? d.classed : classed)
        .html(d => (d.txt) ? d.txt : '');
  ...
  function adaptListener(listener) {
    const isv6 = !('event' in Object.getOwnPropertyNames(selection$1));
    return function (a, b) {
      if (isv6) {
        // d3@v6
        ...
        // d3@v5
        listener.call(this, undefined, a);
      }
    };
  }
  ...
  function slider(orientation, scale) {
    ...
    function slider(context) {
      ...
      if (!scale) {
        scale = domain[0] instanceof Date ? time() : linear$1();
      ...
      value = value.map(function (d) {
        return linear$1().range(domain).domain(domain).clamp(true)(d);
      });
  ...
  // Constants
  const timeStamps = minutes(timeParse('%H:%M:%S')('3:0:0'), timeParse('%H:%M:%S')('15:0:0'),5);
  const visitorsData = [
    1000,
    965.4893846,
    932.1697518,
  ...
  class RevenueData {
    constructor () {
      this._priceStatic = priceStatic;
      this._visitors = [];
      this._rateDynamic = [];
      this._revenueDynamic = [];
      this._data = [];
    }
    ...
    create() {
      ...
      for (let i = 0; i < visitorsData.length; i++) {
        const visitors = Math.floor((visitorsData[i] + Math.floor(Math.random() * (Math.sqrt(visitorsData[i]) / 2))) / 10 + 0.5),
              rateDynamic = (Math.abs(50 - Math.sqrt(visitorsData[i])) + 15) / 10,
              revenueDynamic = this._revenue(visitors, rateDynamic, priceDynamicData[i]);
        this._visitors.push(visitors);
        this._rateDynamic.push(rateDynamic);
        this._revenueDynamic.push(revenueDynamic);
      }
    ...
    headerData () {
      const suma = rollup(this._data, v => sum(v, d => d.revenue), d => d.price),
            sumArray = Array.from(suma, ([key, value]) => ({key, value}));

      const data = sumArray.map(d => {
        return {
            classed: `cell ${d.key}`,
            details: [
              {
                classed: 'details figures',
                values: [
                  {
                    classed: 'value figure',
                    txt: format('($,.0f')(d.value)
                  },
                  {
                    classed: `value variation ${d.key}`,
                    txt: d.key == 'static' ? _variation() : ''
                  },
                ]
              },
              {
                classed: `details text ${d.key}`,
                txt: d.key == 'static' ? 'Static price revenue' : 'Actual revenue'
              }
            ]
          }
      });
      ...
      function _variation () {
        const values = sumArray.sort((a, b) => descending(a.value, b.value)).map(d => d.value),
              variation = (values[1] - values[0])/values[0],
              abs = format(",.1%")(Math.abs(variation));
        return variation < 0 ? `<tspan>&darr;</tspan> ${abs}` : `<tspan>↑</tspan> ${abs}` ;
      }
    ...
    dayTimesData() { 
      const suma = rollups(this._data, v => sum(v, d => d.revenue), d => d.timeDay, d => d.price);
      return suma.map(d => {
        const tl = timeDayBreaks[timeDayLabels.indexOf(d[0])];
        return {
          timeDay: d[0],
          dif: format('$,.0f')(d[1][1][1] - d[1][0][1]),
          timeLimit: tl,
          mid: _midpoint(tl)
        }
      })

      function _midpoint(tl) {
        switch (tl) {
          case timeDayBreaks[0]:
            return timeParse('%H:%M:%S')('2:0:0');
          case timeDayBreaks[1]:
            return timeParse('%H:%M:%S')('4:30:0');
          case timeDayBreaks[2]:
            return new timeParse('%H:%M:%S')('9:0:0');
          case timeDayBreaks[3]:
            return new timeParse('%H:%M:%S')('13:30:0');
          default:
            console.log('default');
        }
      }
    ...
    _buildData() {
      ...
      for (let i = 0; i < timeStamps.length; i++) {
        // get datetime
        const td = bisectRight(timeDayBreaks, timeStamps[i]);
        const rateStatic = this._rateStatic(this._rateDynamic[i], priceDynamicData[i]),
              revenueStatic = this._revenue(this._visitors[i], rateStatic, this._priceStatic);
        this._data.push(
          {
            time: timeStamps[i],
            revenue: this._revenueDynamic[i],
            price: 'dynamic',
            timeDay: timeDayLabels[td]
          },
          {
            time: timeStamps[i],
            revenue: revenueStatic,
            price: 'static',
            timeDay: timeDayLabels[td]
          } 
        );
    ...
    _rateStatic(crd, pd) {
      // crd = rateDynamic
      // pd = priceDynamic
      return (this._priceStatic < pd) ?
        crd * Math.sqrt(pd - this._priceStatic) / pd + crd :
        (-crd) * (Math.pow((this._priceStatic - pd), 1.5)) / pd + crd;
    }

    _revenue(visitors, conversion, price) {
      return Math.floor(visitors * conversion + 0.5) * price / 100;
    }
  }
  ...
  class RevenueChart {
    constructor () {
      this._priceStatic = priceStatic;
      this._container = containers.revenue.chart;
      this._containerHeader = containers.revenue.header;
      this._render = undefined;
      this._dataCreator = undefined;
      this._data = undefined;
    ...
    build() {
      // Data creator class
      this._dataCreator = new RevenueData();
      this._data = this._dataCreator
        .priceStatic(this._priceStatic)
        .create();
      ...
      this._header
        .container(this._containerHeader)
        .title([{txt:'Dynamic pricing vs static price comparison', classed: 'title'}])
        .headerData(this._dataCreator.headerData())
        .init();


      // Render elements
      this._render = new Renders();
      this._render
        .data(this._data)
      ...
      this._render.svg();
      this._render.addTexture(this._texture()); // crea <defs> on resize()
      this._render.slider(this._slider()); // d3-simple-slider
      this._render.axis(this._axis());

      // Append timedays info behind the lines
      this._render.daytime(this._daytimeData());
      this._render.areas(this._areasData());
      this._render.lines(this._linesData()); 


      this._render.addShadowFilter();
    ...
    update() {
      ...
      //update tooltip (en el futuro haré update del tooltip)
      this._tooltip
        .container(this._container)
        .size(this._size())
        .data(this._data)
    ...
    _dataDomains() {
      return {
        time: extent(this._data, d => d.time),
        revenue: [0, max(this._data, d => d.revenue) + 100]
      }
    }
    
    _size () {
      const rawWidth = parseInt(select(this._container).style('width')),
            rawheight = max([(rawWidth * 0.8), 300]);
      return {
        w: rawWidth - margin.left - margin.right,
        h: rawheight - margin.top - margin.bottom,
        slider: (rawWidth - margin.left - margin.right) * 0.6
      }
    }

    _scales () {
      const domains = this._dataDomains(),
            s = this._size();
      return {
        x: time().domain(domains.time).range([0, s.w]),
        y: linear$1().domain(domains.revenue).range([s.h, 0])
      }
    }

    // GENERATORS
    _axis () {
      const step = this._size().w > 400 ? 1 : 2,
            hourTicks = hours(timeParse('%H:%M:%S')('3:0:0'), timeParse('%H:%M:%S')('15:00:00'), step);
      return {
        x: {
          axe: axisBottom(this._scales().x).tickValues(hourTicks).tickFormat(timeFormat('%H:%M')),
          translate: `translate(0, ${this._size().h})`
        },
        y: {
          axe: axisLeft(this._scales().y).tickValues(range(0, 600, 100)),
    ...
    _slider () {
      return sliderBottom()
        .min(sliderRangePrice[0])
        .max(sliderRangePrice[1])
        .width(this._size().slider)
        .tickFormat(format('($.0f')) // dollars format
        .displayFormat(format('($.1f'))
        .tickValues(sliderRangePrice)
        .step(0.1)
        .default(this._priceStatic)
        .handle(
          symbol()
            .type(symbolCircle)
            .size(500)()
    ...
    // DATA 
    _linesData () {
      const x = this._scales().x,
            y = this._scales().y;
      return groups(this._data, d => d.price).map(d => {
        d[1] = d[1].map( v => {
          return {
            x: x(v.time),
            y: y(v.revenue),
    ...
    _areasData () {
      ...
      return groups(this._data, d => d.time).map(d => {
        return {
            x: x(d[0]),
            y1: y(d[1].filter(v => v.price == 'dynamic')[0].revenue),
            y0: y(d[1].filter(v => v.price == 'static')[0].revenue)
          }
      });
    }
    ...
    _daytimeData () {
      ...
      this._dataCreator.dayTimesData().map(d => {
        label.values.push(
          {
            txt: d.timeDay,
            x: x(d.mid),
  ...
  dataVisitors.forEach(d => d.time = timeParse('%H:%M:%S')(d.time));
  ...
  window.addEventListener('DOMContentLoaded', (event) => {
    ...
    // - - -  visitors
    // const visitorsChart = new VisitorsChart();
    // visitorsChart
    //   .data(dataVisitors)
    //   .build();

    // - - -  Revenue
    const revenueChart = new RevenueChart();
    revenueChart.build();
